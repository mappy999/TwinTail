...
namespace Twin.Test
{
	...
	public class HtmlParser
	{
			...
			while (index < html.Length)
			{
				...
				else if (is_match(html, index, "</"))
				{
					...
					if (m.Success)
					{
						...
						if (nodeidx != -1)
						{
							NodesDown(root, nodeidx+1, (__HtmlElement)root[nodeidx]);
						}
						else {
				...
				else if (is_match(html, index, "<"))
				{
					...
					if (m.Success)
					{
						__HtmlElement e = new __HtmlElement(m.Value);
						root.Add(e);
		...
		private int BackFindOpenElement(HtmlNodeCollection nodes, string name)
		{
			...
			{
				__HtmlElement e = nodes[i] as __HtmlElement;
			...
			HtmlAttributeCollection attributes)
		{
			while (index < html.Length)
			{
				...
				if (SkipChars(html, ref index, "=".ToCharArray()))
				{
					...
					attrName = html.Substring(attrSt, index - attrSt).ToUpper();
					index++;
		...
		private void NodesDown(HtmlNodeCollection nodes, int index, __HtmlElement parent)
		{
